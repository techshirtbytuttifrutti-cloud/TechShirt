...
const FinalizeDesignStep: React.FC<FinalizeDesignStepProps> = ({ design }) => {
  ...
  // Only show invoice if approved
  if (billStatus !== "approved") {
    return <div className="p-4 text-gray-600 text-sm">Invoice will be available once the bill is approved.</div>;
  }
  ...
  const finalTotal = final_amount ? final_amount + addons_shirt_price + addons_fee : displayTotal;
        ...
        <div className="flex justify-end mt-3 sm:mt-6">
          <div className="w-full sm:w-1/2 text-xs sm:text-sm space-y-1">
            <div className="flex justify-between border-b border-gray-200 pb-1">
              <span>Subtotal</span>
              <span>₱{final_amount.toLocaleString()}</span>
            </div>
            <div className="flex justify-between border-b border-gray-200 pb-1">
              <span>Tax/VAT (12%)</span>
              <span>₱{(final_amount * 0.12).toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
            </div>
            <div className="flex justify-between border-b border-gray-200 pb-1">
              <span>Total</span>
              <span>₱{final_amount.toLocaleString()}</span>
            ...
            <div className="flex justify-between font-bold text-gray-800 text-sm sm:text-lg bg-gray-50 px-2 sm:px-3 py-1 sm:py-2 rounded-md">
              ...
              <span>₱{final_amount?.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
            </div>
          </div>
        </div>
